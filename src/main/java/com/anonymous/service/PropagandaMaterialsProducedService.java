...
//        propagandaMaterialsProduced.setPropagandaInformation(propagandaInformation);
    ...
    @Override
    public List<Map<String, Integer>> getPropagandaMaterialsProducedByApplicationDate(PropagandaMaterialsProducedStatisticalQuery query) {
        //找出时间间隔内宣传资料制作申请的内容
        List<PropagandaMaterialsProduced> pmps1 = pmpRepository.findByApplicationDateBetween(query.getStartTime(), query.getEndTime());
        //获取宣传资料制作申请内容的宣传类别
        Set<String> propagandaMaterialsContentCategories = new HashSet<>();
        for (PropagandaMaterialsProduced propagandaMaterialsProduced : pmps1) {
            for (PropagandaMaterialsContent propagandaMaterialsContent : propagandaMaterialsProduced.getPropagandaMaterialsContents()) {
                propagandaMaterialsContentCategories.add(propagandaMaterialsContent.getPromotionalCategory().getName());
            }
        }
        //根据宣传类别统计宣传资料制作的所有数量
    ...
    private Map<String, Integer> statisticsData(List<PropagandaMaterialsProduced> propagandaMaterialsProduceds, Set<String> propagandaMaterialsContentCategories) {
        ...
        for (String category : propagandaMaterialsContentCategories) {
            int total = 0;
            for (PropagandaMaterialsProduced propagandaMaterialsProduced : propagandaMaterialsProduceds) {
                for (PropagandaMaterialsContent propagandaMaterialsContent : propagandaMaterialsProduced.getPropagandaMaterialsContents()) {
                    if (category.equals(propagandaMaterialsContent.getPromotionalCategory().getName())) {
                        total = total + propagandaMaterialsContent.getProductionQuantity();
                    }
                }
            }
